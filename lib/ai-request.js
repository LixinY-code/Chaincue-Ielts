...
const LEGACY_AI_HOST = 'sg.uiuiapi.com';
const CURRENT_AI_ORIGIN = 'https://api.uiuihao.com';
...
function resolveBaseUrl() {
  const configuredUrl = process.env.UIUI_BASE_URL || process.env.AI_BASE_URL || DEFAULT_AI_BASE_URL;

  try {
    const url = new URL(configuredUrl);
    if (url.hostname === LEGACY_AI_HOST) {
      return `${CURRENT_AI_ORIGIN}${url.pathname}`.replace(/\/$/, '');
    }
  } catch {
